...
impl<'a> Orchestrator<'a> {
    ...
    pub async fn run(&self, task: &str) -> Result<RunOutcome> {
        ...
        for (idx, step) in plan.steps.iter().enumerate() {
            ...
            let status = self.process_step(task, step, 0, &mut budget).await;
        ...
        if outcome.steps_succeeded > 0 {
            ...
            if let Some(review) = self.final_review(task).await {
    ...
    fn process_step<'s>(
        ...
        Box::pin(async move {
            let ctx = context::file_context(&self.root, &format!("{} {}", step.title, step.detail));
            ...
            // --- 2 & 3. Verify + select ---
            let mut last_failure = match self.collect(step, candidates).await {
                Collected::Passing { scored, tried } => {
                    let best = select_best(self.provider, self.cfg, task, step, &scored).await;
                    let winner = &scored[best];
                    if let Err(e) = apply_blocks(&self.root, &winner.candidate.blocks) {
                        return StepStatus::Failed {
            ...
            for round in 0..self.cfg.scaling.repair_rounds {
                ...
                match self.verify_one(step, &cand).await {
                    Ok(verdict) if verdict.passed => {
                        self.ui.candidate(0, true, &verdict.summary());
                        if let Err(e) = apply_blocks(&self.root, &cand.blocks) {
                            return StepStatus::Failed {
                ...
                if sub.steps.len() > 1 {
                    ...
                    for (i, ss) in sub.steps.iter().enumerate() {
                        ...
                        *budget -= 1;
                        ...
                        if let StepStatus::Failed { .. } =
                            self.process_step(task, ss, depth + 1, budget).await
                        {
                            all_ok = false;
                            break;
                        }
    ...
    /// Verify every candidate; partition into passing (scored) vs. none-passed
    /// (with the best failure log for the repair loop).
    async fn collect(&self, step: &Step, candidates: Vec<Candidate>) -> Collected {
        ...
        for cand in candidates {
            match self.verify_one(step, &cand).await {
                Ok(verdict) => {
                    ...
                    if verdict.passed {
                        let touched: usize = cand
                            .blocks
                            .iter()
                            .map(|b| b.replace.lines().count().max(1))
                            .sum();
                        scored.push(Scored {
                            candidate: cand,
                            verdict,
                ...
                Err(e) => {
                    self.ui.candidate(
                        cand.index,
                        false,
                        &format!("apply error: {}", truncate(&e.to_string(), 80)),
                    );
    ...
    /// Apply one candidate in a fresh sandbox and run the verification gate.
    async fn verify_one(&self, step: &Step, cand: &Candidate) -> Result<Verdict> {
        let sandbox = Sandbox::create(&self.root, &step.title)?;
        apply_blocks(sandbox.path(), &cand.blocks).map_err(|e| anyhow!("{e}"))?;
        Ok(verify(sandbox.path(), &self.cfg.verify, step.check.as_deref()).await)
    }

    async fn final_review(&self, task: &str) -> Option<String> {
